# dFx notional pipeline

Deterministic per-authority notional accounting for Drift Protocol users.

Produces a CSV (`dfx/out/authority_notional.csv`) where each row is one authority
...
Output: `dfx/out/base_snapshot.json` (~40 MB).
...
`dfx/out/authority_notional.csv`.
...
# Phase 1 — fetch on-chain state (writes dfx/out/base_snapshot.json)
bun ./dfx/snapshot.ts \
...
`bun run snapshot` and `bun run revalue` (see root `package.json`) are
shortcuts for the two phases.
...
Both entry scripts import shared helpers from the repo-root `lib/`
(`../lib/*`):
